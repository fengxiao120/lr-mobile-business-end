...
class HomeScene extends PureComponent {
    ...
    constructor(props: Object) {
        ...
        this.props.navigation.setParams({shop:__shop})
    }
    ...
    openingHours() {
        if (this.props.navigation)
            this.props.navigation.navigate('OpeningHours', { shopName: __shop.name, openingHours: __shop.openingHours })
    }

    product() {
        if (this.props.navigation)
            this.props.navigation.navigate('Product', { products: __shop.products, title: __shop.category && __shop.category.name == "Food" ? 'Dishes' : 'Products' })
    }
    ...
    openGps() {
        var coor = __shop.coordinateX + ',' + __shop.coordinateY
            ...
            return (
                <View style={{ flex: 1 }}>
                    ...
                    <ScrollView style={{ backgroundColor: "white" }}>
                        <Text style={{ fontSize: 18, fontWeight: 'bold', padding: 10 }}>{__shop.name}</Text>
                            ...
                            <View style={{ flexDirection: 'row' }}>
                                <View style={{ marginTop: Platform.OS === 'ios' ? 0 : 2 }}>
                                    <StarRating
                                        ...
                                        starSize={16}
                                        rating={__shop.score ? __shop.score.toFixed(1) / 2 : 0}
                                    />
                                </View>
                                <Text style={{ color: '#868686', marginLeft: 2 }}>{__shop.score ? __shop.score.toFixed(1) : '-'}</Text>
                            </View>
                            <Text style={{ color: '#868686', minWidth: 50, marginLeft: 20 }}>{__shop.totalReviews || '0'} reviews</Text>
                            <Text style={{ color: '#868686', minWidth: 100, marginLeft: 20 }}>S${__shop.avgCostPerPerson ? __shop.avgCostPerPerson.toFixed(1) : "-"}/person</Text>
                        </View>
                        <View style={{ flexDirection: 'row', paddingLeft: 10, paddingRight: 10, paddingBottom: 5 }}>
                            <Text style={{ color: '#868686', minWidth: 50 }}>Service: {__shop.score1 ? __shop.score1.toFixed(1) : '-'}</Text>
                            <Text style={{ color: '#868686', minWidth: 50, marginLeft: 20 }}>{(__shop.category && __shop.category.name == "Food") ? "Food" : "Value"}: {__shop.score2 ? __shop.score2.toFixed(1) : '-'}</Text>
                            <Text style={{ color: '#868686', minWidth: 100, marginLeft: 20 }}>{(__shop.category && __shop.category.name == "Food") ? "Atmosphere" : "Quality"}: {__shop.score3 ? __shop.score3.toFixed(1) : '-'}</Text>
                        </View>
                        <View style={{ flexDirection: 'row', paddingLeft: 10, paddingRight: 10, paddingBottom: 5 }}>
                            <Text style={{ color: '#868686', minWidth: 50, flex: 1 }}>{(_.map(__shop.subCategories, 'name')).join(", ")}</Text>
                            <Text style={{ color: '#868686', minWidth: 200 }}>{__shop.district && __shop.district.join(", ")}</Text>
                        </View>
                        <FlatList
                            horizontal={true}
                        ...
                        <View style={{ flexDirection: 'row', minHeight: 50, justifyContent: 'space-between', alignItems: 'center', paddingLeft: 10, paddingBottom: 5, paddingTop: 5 }}>
                            ...
                            <TouchableOpacity style={{ flex: 14, }} onPress={this.openGps.bind(this)}>
                                <Text style={{ fontSize: 14 }}>{__shop.address}</Text>
                            </TouchableOpacity>
                            <Text style={{ flex: 1, fontSize: 20, color: "#868686", fontWeight: '100' }}>|</Text>
                            <TouchableOpacity style={{ alignItems: 'center', flex: 1.5, paddingRight: 10 }} onPress={() => { this.call(__shop.mobile) }}>
                                <Text
                                    style={{ fontSize: 20, backgroundColor: 'rgba(0,0,0,0)' }} >
                        ...
                        {
                            __shop.offers && __shop.offers.length > 0 &&
                            <View>
                        ...
                        {
                            __shop.products && __shop.products.length > 0 &&
                            <TouchableOpacity onPress={this.product.bind(this)}>
                                ...
                                <View style={{ paddingTop: 10, paddingLeft: 10, paddingRight: 10, flexDirection: 'row', justifyContent: "space-between", alignItems: 'center' }}>
                                    <Text style={{ fontSize: 16, fontWeight: 'bold', flex: 14 }}>Recommended {__shop.category && __shop.category.name == "Food" ? 'Dishes' : 'Products'}</Text>
                                    <Text
                                        style={{ fontSize: 15 }} >
                                        <FontAwesome name={'angle-right'} color={'#868686'} size={30} />
                                    </Text>
                                ...
                                <View style={{ padding: 10, flexDirection: 'row' }}>
                                    {
                                        _.filter(__shop.products, (product) => {
                                ...
                                <View style={{ margin: 10, flexDirection: 'row', flexWrap: 'wrap', maxHeight: 40, overflow: 'hidden' }}>
                                    {__shop.products.map(function (product) {
                        ...
                        <View>
                            <TouchableOpacity style={{ paddingTop: 10, paddingLeft: 10, paddingRight: 10, flexDirection: 'row', justifyContent: "space-between", alignItems: 'center' }} onPress={() => {
                                if (this.props.navigation)
                                    this.props.navigation.navigate('Review', { reviews: __shop.reviews, shop: __shop})
                            }}>
                                <Text style={{ fontSize: 16, fontWeight: 'bold', flex: 14 }}>Reviews({__shop.totalReviews})</Text>
                                <Text
                                    style={{ fontSize: 15 }} >
                                    <FontAwesome name={'angle-right'} color={'#868686'} size={30} />
                                </Text>
